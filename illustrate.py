...
def _anim(kind, dur, delay=0.0):
    """Timing as classes, not as a style attribute.

    A strict Content-Security-Policy with a nonce-based `style-src` blocks
    inline style attributes, which silently froze every animation in the
    hosted bundle. Durations and delays are quantised to a half-second grid
    and emitted as utility classes in style.css instead, so the artwork keeps
    moving wherever it is embedded.
    """
    return f'class="il-{kind} il-d{_snap(DURATIONS, dur)} il-y{_snap(DELAYS, delay)}"'


# Half-second steps where the eye can tell them apart, whole seconds where it
# cannot. Snapping to a fixed grid is what lets the timings live in classes.
DURATIONS = [round(3 + 0.5 * i, 1) for i in range(35)] + list(range(21, 122, 2))
DELAYS = [round(0.5 * i, 1) for i in range(17)]
...
def anim_css():
    """The utility classes _anim refers to. build.py appends these to style.css."""
    out = ['/* generated by illustrate.anim_css() - see _anim() for why */']
    out += [f'.il-d{i} {{ animation-duration: {v:g}s; }}' for i, v in enumerate(DURATIONS)]
    out += [f'.il-y{i} {{ animation-delay: {v:g}s; }}' for i, v in enumerate(DELAYS)]
    return '\n'.join(out)
...
def virion(rr, cx, cy, r, col):
    ...
    return (f'<g {_anim("spin", rr.uniform(24, 46), rr.uniform(0, 8))} '
            f'transform-origin="{cx:.0f}px {cy:.0f}px">'
...
def sc_matter(rr):
    ...
    for i in range(7):
        ...
        out.append(f'<g {_anim("spin", rr.uniform(30, 60), rr.uniform(0, 8))} '
                   f'transform-origin="{cx:.0f}px {cy:.0f}px">{shells}</g>'
...
def sc_sun(rr):
    ...
    out.append(f'<g {_anim("spin", 90)} transform-origin="{cx:.0f}px {cy:.0f}px">{flares}</g>')
